...
func (pc *PositionCreate) SetNameposition(s string) *PositionCreate {
	pc.mutation.SetNameposition(s)
	return pc
}
...
func (pc *PositionCreate) Save(ctx context.Context) (*Position, error) {
	if _, ok := pc.mutation.Nameposition(); !ok {
		return nil, &ValidationError{Name: "Nameposition", err: errors.New("ent: missing required field \"Nameposition\"")}
	}
	var (
		err  error
		node *Position
...
func (pc *PositionCreate) createSpec() (*Position, *sqlgraph.CreateSpec) {
	...
	if value, ok := pc.mutation.Nameposition(); ok {
