...
// The PositionassingmentFunc type is an adapter to allow the use of ordinary
// function as Positionassingment mutator.
type PositionassingmentFunc func(context.Context, *ent.PositionassingmentMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f PositionassingmentFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.PositionassingmentMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.PositionassingmentMutation", m)
	}
	return f(ctx, mv)
}
